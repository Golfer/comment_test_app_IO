...
A Rails 8 application for real-time chat and tree-structured comments with
multi-channel notifications and full-text search.

## Stack

- **Rails 8** (Propshaft) + **React** (esbuild) + **Tailwind CSS**
- **PostgreSQL** — primary datastore
- **Redis** — Action Cable pub/sub, Sidekiq queue, Rails cache
...
bin/dev            # runs web + asset watchers + Sidekiq (Procfile.dev)
```

## Architecture

- `app/services/`  — write/business logic (`Messages::CreateService`,
  `Comments::CreateService`, `Chats::CreateChannelService`, `Directs::FindOrCreateService`)
- `app/queries/`   — read/query logic (`Chats::ForUserQuery`, `Messages::ChronologicalQuery`,
  `Comments::TreeQuery`, `Comments::SearchQuery`, `Users::DirectoryQuery`, `Notifications::RecentQuery`)
- `app/notifiers/` — `NewMessageNotifier` + custom delivery methods
  (`DeliveryMethods::InAppCableDelivery`, `DeliveryMethods::PushDeliveryMethod`)
- `app/workers/`   — Sidekiq jobs (`FakeCommentsGenerationWorker`, `ReindexCommentsWorker`)
- `app/channels/`  — `ChatChannel`, `NotificationsChannel`, `PresenceChannel`
...
bin/rails 'fake:comments[1000000,12]'

# Populate Meilisearch afterwards (insert_all bypasses indexing callbacks):
bin/rails fake:reindex
```

Both are Sidekiq jobs on the dedicated low-priority `generation` queue, so they
don't starve interactive work. The generator self-re-enqueues in 100k chunks.
